...
import '../../data/sources/sector_remote_source.dart';
...
class _SectorPageState extends State<SectorPage> {
  final SectorRemoteSource _remoteSource = SectorRemoteSource();
  ...
  Future<List<SectorPerformance>> _load() async {
    final entries = await Future.wait(
      _sectorLeads.entries.map(
        (entry) => _remoteSource.fetchSectorPerformance(entry.key, entry.value),
      ),
    );
    return entries;
  }
